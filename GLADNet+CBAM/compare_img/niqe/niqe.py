...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    img2 = Image.fromarray(img).resize((int(w / 2), int(h / 2)), resample=Image.BICUBIC)
    img2 = np.array(img2)
...
if __name__ == "__main__":
    ...
    ref = cv2.imread('./CBAM/photo_1_glad.png')
    ref = np.array(ref)[:,:,0]
    dis = cv2.imread('./GLADNet/photo_1_glad.png')
    dis = np.array(dis)[:,:,0]
    ho = cv2.imread('./UNet/photo_1_glad.png')
    ho = np.array(ho)[:, :, 0]

    print('NIQE of CBAM image is: %0.3f'% niqe(ref))
    print('NIQE of GLADNet image is: %0.3f'% niqe(dis))
    print('NIQE of UNet image is: %0.3f'% niqe(ho))