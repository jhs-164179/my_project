...
def FG(input_im):
  with tf.compat.v1.variable_scope('FG'):
    ...
    # Channel Attention
    avgpool = GlobalAveragePooling2D(name='_channel_avgpool')(p_conv1)
    maxpool = GlobalMaxPool2D(name='_channel_maxpool')(p_conv1)
    # Shared MLP
    ...
    channel = Reshape((1, 1, nb_filter))(channel)
    channel_out = layers.Multiply()([p_conv1, channel])

    # Spatial Attention
    avgpool = layers.Lambda(tf.reduce_mean, arguments={'axis': 3, 'keepdims': True})(channel_out)
    maxpool = layers.Lambda(tf.reduce_max, arguments={'axis': 3, 'keepdims': True})(channel_out)
    ...
    p_conv1 = layers.Multiply()([channel_out, spatial_out])
    #--------------------------------------------------------------------------------------------------------
    p_conv2 = tf.compat.v1.layers.conv2d(p_conv1,  64, 3, 2, padding='same', activation=tf.nn.relu) # 24

    # Channel Attention
    avgpool = GlobalAveragePooling2D(name='_channel_avgpool')(p_conv2)
    maxpool = GlobalMaxPool2D(name='_channel_maxpool')(p_conv2)
    # Shared MLP
    Dense_layer1 = Dense(nb_filter // 8, activation='relu')
    Dense_layer2 = Dense(nb_filter, activation='relu')
    avg_out = Dense_layer2(Dense_layer1(avgpool))
    ...
    channel = Activation('sigmoid')(channel)
    channel = Reshape((1, 1, nb_filter))(channel)
    channel_out = layers.Multiply()([p_conv2, channel])
    ...
    p_conv2 = layers.Multiply()([channel_out, spatial_out])

    # --------------------------------------------------------------------------------------------------------
    p_conv3 = tf.compat.v1.layers.conv2d(p_conv2,  64, 3, 2, padding='same', activation=tf.nn.relu) # 12
    
    # Channel Attention
    avgpool = GlobalAveragePooling2D(name='_channel_avgpool')(p_conv3)
    maxpool = GlobalMaxPool2D(name='_channel_maxpool')(p_conv3)
    # Shared MLP
    Dense_layer1 = Dense(nb_filter // 8, activation='relu')
    Dense_layer2 = Dense(nb_filter, activation='relu')
    avg_out = Dense_layer2(Dense_layer1(avgpool))
    ...
    channel = Activation('sigmoid')(channel)
    channel = Reshape((1, 1, nb_filter))(channel)
    channel_out = layers.Multiply()([p_conv3, channel])
    ...
    # CBAM_out
    p_conv3 = layers.Multiply()([channel_out, spatial_out])

    # --------------------------------------------------------------------------------------------------------
    p_conv4 = tf.compat.v1.layers.conv2d(p_conv3,  64, 3, 2, padding='same', activation=tf.nn.relu) # 6

    # Channel Attention
    avgpool = GlobalAveragePooling2D(name='_channel_avgpool')(p_conv4)
    maxpool = GlobalMaxPool2D(name='_channel_maxpool')(p_conv4)
    # Shared MLP
    Dense_layer1 = Dense(nb_filter // 8, activation='relu')
    Dense_layer2 = Dense(nb_filter, activation='relu')
    avg_out = Dense_layer2(Dense_layer1(avgpool))
    ...
    channel = Activation('sigmoid')(channel)
    channel = Reshape((1, 1, nb_filter))(channel)
    channel_out = layers.Multiply()([p_conv4, channel])
    ...
    p_conv4 = layers.Multiply()([channel_out, spatial_out])

    # --------------------------------------------------------------------------------------------------------
    p_conv5 = tf.compat.v1.layers.conv2d(p_conv4,  64, 3, 2, padding='same', activation=tf.nn.relu) # 3

    # Channel Attention
    avgpool = GlobalAveragePooling2D(name='_channel_avgpool')(p_conv5)
    maxpool = GlobalMaxPool2D(name='_channel_maxpool')(p_conv5)
    # Shared MLP
    Dense_layer1 = Dense(nb_filter // 8, activation='relu')
    Dense_layer2 = Dense(nb_filter, activation='relu')
    avg_out = Dense_layer2(Dense_layer1(avgpool))
    ...
    channel = Activation('sigmoid')(channel)
    channel = Reshape((1, 1, nb_filter))(channel)
    channel_out = layers.Multiply()([p_conv5, channel])
    ...
    # CBAM_out
    p_conv5 = layers.Multiply()([channel_out, spatial_out])

    # --------------------------------------------------------------------------------------------------------
    p_conv6 = tf.compat.v1.layers.conv2d(p_conv5,  64, 3, 2, padding='same', activation=tf.nn.relu) # 1

    # Channel Attention
    avgpool = GlobalAveragePooling2D(name='_channel_avgpool')(p_conv6)
    maxpool = GlobalMaxPool2D(name='_channel_maxpool')(p_conv6)
    # Shared MLP
    Dense_layer1 = Dense(nb_filter // 8, activation='relu')
    Dense_layer2 = Dense(nb_filter, activation='relu')
    avg_out = Dense_layer2(Dense_layer1(avgpool))
    ...
    channel = Activation('sigmoid')(channel)
    channel = Reshape((1, 1, nb_filter))(channel)
    channel_out = layers.Multiply()([p_conv6, channel])
    ...
    # CBAM_out
    p_conv6 = layers.Multiply()([channel_out, spatial_out])
    # --------------------------------------------------------------------------------------------------------
